    ...
    public function search(Request $request)
    {
        ...
        if ($name) {
            $first = DB::table('products')
                ->selectRAW(' "products" AS type, products.p_id ,products.p_gname,products.p_id as codigo')
                ->whereRAW("products.p_gname like '%$name%'");

            $second = DB::table('customers')
                ->selectRAW('"customers" AS type, customers.number,customers.name,customers.id as codigo')
                ->whereRAW("customers.name like '%$name%' OR customers.number like '%$name%'");

            $search = DB::table('orders')
                ->selectRAW('"orders" AS type, orders.order_code AS id,orders.invoice_no AS name,orders.id as codigo')
