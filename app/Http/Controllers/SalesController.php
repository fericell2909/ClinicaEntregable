...
class SalesController extends Controller
{
    ...
    {
        $orders = Orders::Listar_Documento($codigo_documento);
        $ordendetalles = Orders::Listar_Detalle_Documento($codigo_documento);
        $empresas =  Settings::Listar_Datos_Empresa();
        //var_dump($orders);
        $vistaurl="invoice.documento";
    ...
    private function ImpimirPDF($orden,$detalle_orden,$empresas,$vistaurl,$tipo = 1)
    {
        ...
        $documento = strval($orders[0]->invoice_no); 
    ...
    public function create()
    {
      ...
      //var_dump($tipospagos);
        ...
        return view('sales.create', compact('product','tiposdocumentos','tipospagos'));
    ...
    public function store(Request $request)
    {
        ...
        if (!(Orders::ValidaClienteTipoDocumento($data))) {
            
            session()->flash('warning', 'El Cliente Seleccionado no Corresponde con el Tipo de Documento.');
            return $this->Create();
        ...
        Orders::ValidaStockProductoVenta($data,$bvalidacion,$bmensajevalidacion);

        // No hay Stock Disponible.
        if (!$bvalidacion) {
            session()->flash('warning', $bmensajevalidacion);    
            return $this->Create();
        }

        $bresultado = Orders::GuardarDocumento($data,Auth::user()->id,Auth::user()->email,$codigo_generado);
        ...
        if ($bresultado === true) {
           ...
           // FacturacionElectronica::GeneraDocumentoElectronico($data,$codigo_generado); 
            ...
            //session()->flash('Correcto', $resultado_fact);
            //return redirect()->route('sales.show',$codigo_generado);
    ...
    public function show($id)
    {
         ...
         $orders = Orders::Listar_Documento($id);
         $ordendetalles = Orders::Listar_Detalle_Documento($id);
         $empresas =  Settings::Listar_Datos_Empresa();
        ...
        return view('sales.show', ['orders' => $orders,'ordendetalles'=>$ordendetalles,'empresas'=>$empresas]);
    }
